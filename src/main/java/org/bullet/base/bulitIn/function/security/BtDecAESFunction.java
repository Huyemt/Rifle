    ...
    public Object eval(LinkedHashMap<String, Object> args) throws BulletException {
        ...
        try {
            if (!(iv instanceof BtNull) && ((String) iv).length() > 0)
                return Crypto4J.AES.decrypt(content.toString(), key.toString(), getMode((int) ((BtNumber) mode).toLong()), getPadding((int) ((BtNumber) padding).toLong()), iv.toString());
            else
                return Crypto4J.AES.decrypt(content.toString(), key.toString(), getMode((int) ((BtNumber) mode).toLong()), getPadding((int) ((BtNumber) padding).toLong()));
        } catch (Exception e) {
    ...
    private AES.Mode getMode(int n) throws BulletException {
        switch (n) {
            ...
            case 4:
                return AES.Mode.CFB;
            default:
                throw new BulletException("Decryption mode not supported");
        }
    }
